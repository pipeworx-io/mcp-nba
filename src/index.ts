...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NBA');
}
...
function extractKey(args: Record<string, unknown>): string {
  ...
  if (!key || typeof key !== 'string' || !key.trim()) {
    throw new Error('BallDontLie API key required. The free tier needs a key — get one in 30 seconds at https://www.balldontlie.io/ and pass via _apiKey.');
  }
  return key;
}

function buildHeaders(): HeadersInit {
  return _currentApiKey ? { Authorization: _currentApiKey } : {};
}

async function checkResponse(res: Response, op: string): Promise<void> {
  if (res.ok) return;
  if (res.status === 401) {
    throw new Error(`BallDontLie rejected the API key (401). Check that _apiKey is your real key from https://www.balldontlie.io/ — the free tier works for search_players + get_teams + get_games.`);
  }
  if (res.status === 429) {
    throw new Error(`upstream_throttled: BallDontLie rate limit hit (429). Free tier is 5 req/min; upgrade at https://www.balldontlie.io/ for higher limits or back off.`);
  }
  if (res.status === 403) {
    // BallDontLie returns 403 with a body that often contains "No approval
    // received" when calling a tier-gated endpoint with an insufficient key.
    // Production feedback 2026-06-08 caught this — agent called get_player
    // (the /players/{id} endpoint, which IS gated to ALL-STAR tier or higher)
    // with a free-tier key and got an opaque "No approval received".
    let bodyHint = '';
    try { bodyHint = (await res.text()).slice(0, 120); } catch { /* ignore */ }
    throw new Error(`BallDontLie endpoint not in your tier (403)${bodyHint ? `: ${bodyHint}` : ''}. The free tier covers search_players + get_teams + get_games; get_player (full profile) and season averages require the ALL-STAR tier ($9.99/mo) at https://www.balldontlie.io/. Pass a higher-tier _apiKey or use search_players which returns basic profile (no per-season PPG/RPG/APG).`);
  }
  throw new Error(`BallDontLie API error during ${op}: ${res.status}`);
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        _apiKey: { type: 'string', description: 'BallDontLie API key' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        _apiKey: { type: 'string', description: 'BallDontLie API key (free tier: https://www.balldontlie.io/)' },
      },
      required: ['_apiKey'],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  _currentApiKey = extractKey(args);
  switch (name) {
    case 'search_players':
...
async function searchPlayers(query: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/players?${params}`, { headers: buildHeaders() });
  await checkResponse(res, 'search_players');
...
async function getPlayer(id: number) {
  const res = await pwFetch(`${BASE_URL}/players/${id}`, { headers: buildHeaders() });
  await checkResponse(res, 'get_player');
...
async function getTeams() {
  const res = await pwFetch(`${BASE_URL}/teams`, { headers: buildHeaders() });
  await checkResponse(res, 'get_teams');
...
async function getGames(season: number, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/games?${params}`, { headers: buildHeaders() });
  await checkResponse(res, 'get_games');
